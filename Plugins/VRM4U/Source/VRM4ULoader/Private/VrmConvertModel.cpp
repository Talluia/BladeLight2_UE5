...
bool VRMConverter::ConvertModel(UVrmAssetListObject *vrmAssetList, const aiScene *mScenePtr) {
	...
	static int boneOffset = 0;
	{
		...
		//k->MergeAllBonesToBoneTree(src);
		{
			...
			VRMSkeleton::readVrmBone(const_cast<aiScene*>(mScenePtr), boneOffset, VRMGetRefSkeleton(sk_tmp));
		...
		for (int i = 0; i < VRMGetMaterials(sk).Num(); ++i) {
			...
			VRMGetMaterials(sk)[i].MaterialSlotName = UTF8_TO_TCHAR(mScenePtr->mMaterials[i]->GetName().C_Str());
		}
			...
			for (int meshID = 0; meshID < result.meshInfo.Num(); ++meshID) {
				...
				for (int i = 0; i < mInfo.Vertices.Num(); ++i) {
					...
					*meshS = softSkinVertexLocalZero;
					...
					if (VRMConverter::Options::Get().IsVRM1Model() || VRMConverter::Options::Get().IsPMXModel() || VRMConverter::Options::Get().IsBVHModel()) {
