...
bool VrmJson::init(const uint8* pData, size_t size) {
	...
	v.push_back(0);
	jsonData = nlohmann::ordered_json::parse(&v[0]);

	return true;
}


void VrmJsonTest(const uint8* pData, size_t size) {
}

bool VRMIsVRM1(const uint8* pData, size_t size) {
	...
	memcpy(&v[0], pData + c_start, c_end - c_start);
	v.push_back(0);
	nlohmann::json j = nlohmann::json::parse(&v[0]);

	if (j.contains("extensions")) {
		if (j["extensions"].contains("VRMC_vrm")) {
			return true;
		}
	}
	return false;
