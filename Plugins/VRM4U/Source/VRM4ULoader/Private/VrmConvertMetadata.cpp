...
bool VRMConverter::ConvertVrmMeta(UVrmAssetListObject *vrmAssetList, const aiScene *mScenePtr, const uint8* pData, size_t dataSize) {
	...
	// bone
	if (VRMConverter::Options::Get().IsVRM1Model()){
		if (pData && dataSize) {
			VrmJson j;
			j.init(pData, dataSize);
			auto key1 = nlohmann::ordered_json::json_pointer("/extensions/VRMC_vrm/humanoid/humanBones");
			auto humanBone = j.jsonData[key1];

			auto key2 = nlohmann::ordered_json::json_pointer("/nodes");
			auto origBone = j.jsonData[key2];

			for (auto& g : humanBone.items()) {
				auto nameFrom = g.key();
				int node = g.value()["node"].get<int>();

				std::string nameTo = "";
				if (node >= 0 && node < origBone.size()) {
					nameTo = origBone[node]["name"].get<std::string>();
				}

				m->humanoidBoneTable.Add(UTF8_TO_TCHAR(nameFrom.c_str())) = UTF8_TO_TCHAR(nameTo.c_str());
			}
		}
	} else {
