...
bool FVrmAnimInstanceCopyProxy::Evaluate(FPoseContext& Output) {
	...
	for (const auto &t : VRMUtil::vrm_humanoid_bone_list) {
		...
		if (BoneCount == 1) {
			...
			dstTrans.SetTranslation(dstRefTrans.GetLocation() + FMath::Lerp(diff, diff * HeightScale, CenterLocationScaleByHeightScale));
			if (dstIndex == 0) {
	...
	if (bUniqueRootBone){
		...
		auto t = srcTrans;
		t.SetTranslation(FMath::Lerp(t.GetTranslation(), t.GetTranslation() * HeightScale, CenterLocationScaleByHeightScale));
		pose[bi] = t;
	}
